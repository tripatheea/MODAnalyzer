...
class Event {
	...
	public:
		...
		vector<PseudoJet> jets(int event_number, JetDefinition jet_def, double pt_cut);	// JetDefinition, pt_cut (Fastjet)
...
vector<PseudoJet> Event::jets(int event_number, JetDefinition jet_def, double pt_cut) {
	vector<PseudoJet> particles = this->particles_four_vectors();
...
void Event::write_to_file(string filename) {
	...
	for (int i = 0; i < particles.size(); i++) {
		...
		file_to_write << this->type_of_particles() << " " <<  four_vector[0] << "     " << four_vector[1] << "     " << four_vector[2] << "     " << four_vector[3] << "     " << current_particle.mass() << "          " << current_particle.pdgId() << endl;
	}
	...
	for(int i = 0; i < triggers.size(); i++) {
		...
		file_to_write << "trig   " << current_trigger.name() << "         " << prescales.first << "          " << prescales.second << "          " << current_trigger.fired() << endl;
	}
...
Trigger Event::assigned_trigger() {
	...
	*/
	...
	if (trigger_to_use.length() != 0) {
		...
		Trigger selected_trigger = Event::trigger_by_name(trigger_to_use);
