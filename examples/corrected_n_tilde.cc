...
void corrected_n_tilde(MOD::Event & event_being_read, ofstream & output_file);
...
int main(int argc, char * argv[]) {
   ...
   while( event_being_read.read_event(data_file) && ( event_serial_number <= number_of_events_to_process ) ) {
      ...
      corrected_n_tilde(event_being_read, output_file);
...
void corrected_n_tilde(MOD::Event & event_being_read, ofstream & output_file) {
   ...
   vector<MOD::CalibratedJet> corrected_jets = event_being_read.corrected_calibrated_jets_ak5();

   // Create a new event with this corrected jets' constituent PFCandidates.
